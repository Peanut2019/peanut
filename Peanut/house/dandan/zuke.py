...
def home_message():
    for i in new_home_url:
        for b in i:
            ...
            district = home_mess.xpath("//span[@class='c-333']/text()")[7]
            district = ' '.join(district)
            print(district)
            house = home_mess.xpath("//span[@class='c-333 left']/text()")
            house = ' '.join(house)
            print(house)
            monthly = home_mess.xpath("//span[@class='f26 bold']/text()")
            monthly = ' '.join(monthly)+'元/月'
            print(monthly)
            house_info = home_mess.xpath("//span[@class='c-333']/text()")
            house_info = ' '.join(house_info)
            print(house_info)
            agent = home_mess.xpath("//p[@class='mt4 f18 c-333']/text()")
            agent_phone = home_mess.xpath("//span[@class='f16 c-ff5555']/text()")
            agent = ' '.join(agent)+':'+' '.join(agent_phone)
            print(agent)
            imgs = home_mess.xpath("//img[@class='zooming-switch']/@src")
            imgs = ' '.join(imgs)
            print(imgs)
            traffic = home_mess.xpath("//div[@class='plr20 c-333 f14']/text()")
